...
unused_letters = let
	alphabet[findall(iszero, sample_freqs)]
end
...
th_frequency = sample_freq_matrix[findfirst(x->x=='t', alphabet), findfirst(x->x=='h', alphabet)][1]

# ╔═╡ 1f94e0a2-f9ab-11ea-1347-7dd906ebb09d
md"""👉 What about `"ht"`?"""

# ╔═╡ 41b2df7c-f931-11ea-112e-ede3b16f357a
ht_frequency = sample_freq_matrix[findfirst(x->x=='h', alphabet), findfirst(x->x=='t', alphabet)][1]
...
begin
	sample_freq_doubles = diag(sample_freq_matrix)
	double_letters = alphabet[findall(x->x!=0, sample_freq_doubles)]
end
...
most_likely_to_follow_w = alphabet[argmax(sample_freq_matrix[findfirst(x->x=='w', alphabet), :])]
...
most_likely_to_precede_w = alphabet[argmax(sample_freq_matrix[1:26, findfirst(x->x=='w', alphabet)])]
#=
Otherwise, the answer is ' ', the blank space.
=#
...
emma_count = count(x->x=="Emma", emma_words)
