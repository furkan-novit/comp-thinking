...
begin
	...
	function quantize(color::AbstractRGB)
		...
		RGB_type = typeof(color)
		return RGB_type(quantize(color.r), quantize(color.g), quantize(color.b))
	end
...
function invert(color::AbstractRGB)
	...
	return RGB_type(1 - color.r, 1 - color.g, 1 - color.b)
end
...
begin
	function noisify(x::Number, s)
		...
		return min(max(x + rand((-s, s)), 0), 1)
	end
	
	function noisify(color::AbstractRGB, s)
		# you will write me in a later exercise!
		return typeof(color)(noisify(color.r, s), noisify(color.g, s), noisify(color.b, s))
	end
...
function blur_1D(v, l)
	v_blurred = copy(v)
	for i in 1:length(v)
		v_blurred[i] = mean([extend(v, j) for j in i-l:i+l])
	end
	return v_blurred
end
...
# ╔═╡ 96c4bdc3-dc41-4fd5-8957-335f924f3a59
begin
	blurred_v = blur_1D(v, l_box)
	colored_line(blurred_v)
end

# ╔═╡ 0d096afe-1add-4069-b748-ccdab1fdc426
colored_line(v)
...
function convolve_vector(v, k)
	...
	convolved_v = copy(v)
	for i in 1:length(v)
		convolved_v[i] = sum([extend(v, i + l - j + 1) * k[j] for j in 1:length(k)])
	end
	return convolved_v
end
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	for i=1:size(M)[1], j=1:size(M)[2]
		convolved_M[i, j] = sum([extend_mat(M, i + l1 - m + 1, j + l2 - n + 1) * K[m, n] for m=1:num_rows, n=1:num_columns])
	end
	return convolved_M
end
...
function with_gaussian_blur(image)
	...
	l = div(n, 2)
	x = -l:1:l
	z = repeat(x.*x, 1, n) + repeat(x'.*x', n, 1)
	kernel = (1 / sqrt(2 * pi)) * exp.(z/2)
