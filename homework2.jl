...
student = (name = "Furkan", kerberos_id = "furk")
...
function greedy_seam(energies, starting_pixel::Int)
	rows = size(energies, 1)
	cols = size(energies, 2)
	seam = zeros(Int, rows)
	seam[1] = starting_pixel # store first row, column!
	
	for i=2:rows
		next_cols = seam[i-1] .+ [-1,0,1]
		if minimum(next_cols)<1
			next_cols[1] = 1
		elseif maximum(next_cols)>cols
			next_cols[3] = cols
		end
		
		seam[i] = next_cols[findmin(@view energies[i,next_cols])[2]]
	end
	return seam
end
...
function least_energy(energies, i, j)
	...
	least, _ = least_energy(energies, i + 1, j)
	next_j = j
	if j > 1
		left_least, _ = least_energy(energies, i + 1, j - 1)
		if left_least < least
			least = left_least
			next_j = j - 1
		end
	elseif j < cols
		right_least, _ = least_energy(energies, i + 1, j + 1)
		if right_least < least
			least = right_least
			next_j = j + 1
		end
	end
	return energies[i, j] + least, next_j
end
...
function recursive_seam(energies, starting_pixel)
	...
	for i=2:m
		_, seam[i] = least_energy(energies, i - 1, seam[i - 1])
	end
	return seam
end
...
function memoized_least_energy(energies, i, j, memory)
	...
	m, n = size(energies)
	if i == m
		memory[key] = energies[i, j]
		return memory[key]
	end
	
	least = memoized_least_energy(energies, i + 1, j, memory)
	if j > 1
		left_least = memoized_least_energy(energies, i + 1, j - 1, memory)
		if left_least < least
			least = left_least
		end
	elseif j < n
		right_least = memoized_least_energy(energies, i + 1, j + 1, memory)
		if right_least < least
			least = right_least
		end
	end
	memory[key] = energies[i, j] + least
	return memory[key]
end
...
function recursive_memoized_seam(energies, starting_pixel)
	...
	for i=2:m
		least = memoized_least_energy(energies, i, current_pixel, memory)
		if current_pixel > 1
			left_least = memoized_least_energy(energies, i, current_pixel - 1, memory)
			if left_least < least
				least = left_least
				current_pixel -= 1
			end
		elseif current_pixel == n
			right_least = memoized_least_energy(energies, i, current_pixel + 1, memory)
			if right_least < least
				least = right_least
				current_pixel += 1
			end
		end
		seam[i] = current_pixel
	end
	return seam
end
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	least = matrix_memoized_least_energy(energies, i + 1, j, memory)
	if j > 1
		left_least = matrix_memoized_least_energy(energies, i + 1, j - 1, memory)
		if left_least < least
			least = left_least
		end
	elseif j < n
		right_least = matrix_memoized_least_energy(energies, i + 1, j + 1, memory)
		if right_least < least
			least = right_least
		end
	end
	memory[i, j] = energies[i, j] + least
	return memory[i, j]
end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	current_pixel = starting_pixel
	for i=2:m
		least = matrix_memoized_least_energy(energies, i, current_pixel, memory)
		if current_pixel > 1
			left_least = matrix_memoized_least_energy(energies, i, current_pixel - 1, memory)
			if left_least < least
				least = left_least
				current_pixel -= 1
			end
		elseif current_pixel == n
			right_least = matrix_memoized_least_energy(energies, i, current_pixel + 1, memory)
			if right_least < least
				least = right_least
				current_pixel += 1
			end
